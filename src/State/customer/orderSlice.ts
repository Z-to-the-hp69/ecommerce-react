...
import { Order, OrderState, OrderItem } from "../../types/orderTypes";
import { stat } from "fs";
...
//         {
//           ...address,
//           paymentMethod: paymentGateway, // Add paymentMethod to the body
//         },
//         {
//           headers: { Authorization: `Bearer ${jwt}` },
//         }
...
export const paymentSuccess = createAsyncThunk<
  any,
  { paymentId: string; jwt: string; paymentLinkedId: string },
  { rejectValue: string }
>(
  "orders/paymentSuccess",
  async ({ paymentId, jwt, paymentLinkedId }, { rejectWithValue }) => {
    try {
      const response = await api.get(`/api/payment/${paymentId}`, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
        params: { paymentLinkedId },
      });
